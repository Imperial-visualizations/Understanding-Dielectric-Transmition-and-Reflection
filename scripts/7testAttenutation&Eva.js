...
$(window).on('load', function() {//main
    ...
    function getData_wave_transmitted(attenuation){//produce data of transmitted wave
        let z,z_square = [];
        let E_0_t = transmit();
        let k_y = Math.sin(theta_i)*k_1;
        ...
        if (isNaN(theta_t) === true){//evanescent waves produced
            ...
            for (let v=0;v < y_data.length ;v++) {
                ...
                z_square.push(math.dotMultiply(attenuation,z_row));
            }
        }
        return z_square;
    }
    ...
    function plot_data() {//plot traces
        ...
        let transmitted_wave = {
            ...
            x: x_data_t,
            y: y_data,
            z: getData_wave_transmitted(properties[2]),
            type: 'surface',
            name:"Transmitted",
            showscale: false,
        };
